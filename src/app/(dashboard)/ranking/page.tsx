...
import { Medal, Award, Crown, GraduationCap, ClipboardCheck } from "lucide-react";
import { getLeaderboard, getTestLeaderboard } from "@/actions/exam";
...
type Tab = "assignment" | "test";

function mapEntries(data: Record<string, unknown>[]): LeaderboardEntry[] {
...
export default function RankingPage() {
  ...
  useEffect(() => {
    async function fetchRanking() {
      const [assignData, testData] = await Promise.all([
        getLeaderboard(),
        getTestLeaderboard(),
      ]);
      setAssignmentEntries(mapEntries(assignData));
      setTestEntries(mapEntries(testData));
      setLoading(false);
    }
    fetchRanking();
  }, []);
  ...
  const entries = tab === "assignment" ? assignmentEntries : testEntries;
